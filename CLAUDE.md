...
- **Content-addressed blob store**: raw `.eml` files at `<message_dir>/sha256/<ab>/<cd>/<hash>.eml`. Blobs are written atomically (temp file + rename). Multiple recipients share one blob. Stored in the `messages-data` Docker named volume, mounted into the container at `/var/lib/rookery/messages` (the binary's hardcoded default). The container runs as the distroless `nonroot` UID 65532; Docker creates the volume owned by that UID, so there is no host-side directory, `chown`, or UID/GID plumbing. `rookery backup` archives the volume through a helper container (the same pattern used for `caddy-data`). `message_dir` remains overridable in `rookery.toml` for non-container use (e.g. `go run`).

...
- **Content-addressed blob store**: raw `.eml` files at `<message_dir>/sha256/<ab>/<cd>/<hash>.eml`. Blobs are written atomically (temp file + rename). Multiple recipients share one blob. Stored in the `messages-data` Docker named volume, mounted into the container at `/var/lib/rookery/messages` (the binary's hardcoded default). The container runs as root (like the other service containers), so it writes to the root-owned volume directly — there is no host-side directory, `chown`, or UID/GID plumbing. `rookery backup` archives the volume through a helper container (the same pattern used for `caddy-data`). `message_dir` remains overridable in `rookery.toml` for non-container use (e.g. `go run`).
- **Export archive store**: encrypted `.tar.gpg` archives at `<message_dir>/exports/<job-id>.tar.gpg`. Lives inside the `messages-data` volume so it requires no separate volume — `store.Open` creates the subdirectory at startup. Archives expire after 24 hours; a background cleanup worker deletes them. Excluded from `rookery backup` (temporary files, not user data).
...
- `.env`: `ROOKERY_DB_PASSWORD`, `POSTGRES_PASSWORD`, `ROOKERY_MASTER_KEY`, `ROOKERY_SESSION_KEY`. Included in `./rookery backup` — losing `ROOKERY_MASTER_KEY` bricks DKIM keys.
...
- Export jobs tracked in the `export_jobs` table (migration 0007). Archives stored in the `exports/` subdirectory of `message_dir` (the `messages-data` volume). Hourly cleanup worker in `CleanupExpiredExports` removes files after 24h expiry.

...
- HTTP server on port 80 (chi router, `net/http`)
...
- `.env` (generated by `run.sh`): `ROOKERY_DB_PASSWORD`, `POSTGRES_PASSWORD`, `ROOKERY_MASTER_KEY`, `ROOKERY_SESSION_KEY`. Back up `ROOKERY_MASTER_KEY` — losing it bricks DKIM keys and ACME credentials.

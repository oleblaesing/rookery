...
`run.sh` is a wrapper around `docker compose` that generates `.env` (random secrets) before Compose starts. Always use it instead of `docker compose` directly.

```sh
# Start dev stack (rookery + postgres + mailpit SMTP sink at http://localhost:8025)
# Web UI is at http://localhost:8080
./run.sh --profile dev up --build

# Start with HTTPS (production) — Caddyfile is auto-generated from rookery.toml on first run
./run.sh --profile prod up --build

# Run the Go test suite
./run.sh --profile test run --rm test

# Run go vet
./run.sh --profile lint run --rm lint
```

Before the first run: `cp rookery.toml.example rookery.toml` and edit `domain`.

**Operator scripts** run in the `postgres` container (the rookery container is distroless — no shell):
```sh
docker compose exec -T postgres sh /scripts/new-invite.sh
./scripts/send-test-mail.sh          # inject a test message via mailpit
```
...
- `rookery.toml` (from `rookery.toml.example`): domain, HTTP, SMTP, storage, policy settings.
- `Caddyfile` (auto-generated by `run.sh` from domain + contact email in `rookery.toml`): Caddy reverse-proxy config for TLS/HTTPS. Generated on first `--profile prod` run; gitignored.
- `.env` (generated by `run.sh`): `ROOKERY_DB_PASSWORD`, `POSTGRES_PASSWORD`, `ROOKERY_MASTER_KEY`, `ROOKERY_SESSION_KEY`. Back up `ROOKERY_MASTER_KEY` — losing it bricks DKIM keys.

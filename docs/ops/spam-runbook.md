...
1. You must be running the prod profile (Caddy), because the submission listener
   ...
   second ACME client. Caddy stores its certs as root-owned `0600` files; rookery
   also runs as root, so it reads them from the shared `caddy-data` volume
   directly — no permission setup needed.

...
**Deliverable:** `docker compose --profile dev up --build` boots an empty server with `/healthz` (and mailpit for SMTP testing). The project builds from a clean checkout on any host with only `docker` installed. ADRs committed.
...
### Phase 3 — TLS and HTTPS for the primary domain (≈1–2 weeks)
...
- **ACME integration via `certmagic`.** At startup the server provisions a TLS certificate for the primary domain from Let's Encrypt using the HTTP-01 challenge. The ACME account is created once and stored in Postgres (encrypted with the server master key, §11.6). Required config: `acme_email` in `rookery.toml` — the Let's Encrypt contact address for expiry notices. Auto-renewal is handled transparently by `certmagic`. The server logs cert status on startup and on every renewal.
- **Port 80 — ACME challenge handler + redirect.** Port 80 handles HTTP-01 ACME challenges and redirects all other traffic to HTTPS. Before the first certificate is provisioned the server falls back to serving the web UI over HTTP (Phase 1–2 behaviour), so operators can complete initial setup before TLS is live.
- **Web UI on port 443 (HTTPS).** Session cookies gain their `Secure` flag. HSTS header on all HTTPS responses (`Strict-Transport-Security: max-age=63072000`; no `preload` in v1 — preload is a long-term commitment that an operator may need to back out of during incident response).
- **SMTP inbound STARTTLS on port 25.** The provisioned cert is used. Well-behaved senders upgrade to TLS; unencrypted fallback is still accepted because the rest of the internet still sends some of it.
- **SMTP submission ports 465 (implicit TLS) and 587 (STARTTLS) become operational.** These ports were started in Phase 2 but unusable without a TLS cert. With Phase 3's cert in place they accept authenticated submission (§11.4).
- **`openpgpkey.<domain>` HTTPS.** The WKD advanced method endpoint (live since Phase 1) requires HTTPS per spec. `certmagic` provisions a cert for `openpgpkey.<domain>` alongside the primary domain cert; once Phase 3 ships, WKD becomes spec-compliant for the primary domain. Per-custom-domain WKD TLS arrives in Phase 4.
- **Certificate expiry monitoring.** The server logs a warning ≥14 days before cert expiry. This becomes a Prometheus metric in Phase 5.
- **One new Go dependency: `certmagic`.** Justified by scope — a correct, production-tested ACME client is not something we reimplement in a few hundred lines. ADR-0024 records the TLS and ACME design decisions.

**Deliverable:** The web UI is reachable at `https://<domain>` with a valid Let's Encrypt certificate. Session cookies are Secure. Inbound SMTP on port 25 offers STARTTLS with a valid cert. Submission on 465/587 is operational. Certificate renewal is automatic. The primary-domain WKD endpoint is spec-compliant HTTPS.
...
- **Registration.** Invite-only, always. There is no open-registration mode and no config knob to enable one — the abuse risk on a pseudonymous, IP-log-free instance is not a trade-off worth offering. Invite tokens are rows in an `invites` table; the operator creates them via `./scripts/new-invite.sh` (a thin `psql INSERT` wrapper). User-issued invites are a Phase 8+ option; not in v1. ADR-0008.
...
- `ADR-0024` — DNS, TLS, WKD: advanced-method WKD only, ACME HTTP-01 via `certmagic`, MTA-STS mode handling (testing → enforce), DKIM ed25519+RSA dual selectors. **Written; needed before Phase 3.**

...
|---|---|---|
| `rookery init [--domain X] [--email Y] [--name N] [--user U]` | User-local bootstrap from a clone. Copies `rookery.toml.example` → `rookery.toml` if missing (flags or interactive prompts fill `domain`, `contact_email`, `instance_name`); generates `.env` (random secrets) if missing; generates `Caddyfile` if missing; stages `./rookery.service` with `User=` from `--user` (default: `whoami`) if missing. Never overwrites; safe to re-run. No `sudo`. | `openssl rand`, `cp`, a few `sed`s, a `printf` |

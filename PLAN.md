...
4. **`docker compose up -d`.** Server starts, opens ports 25 (SMTP) and 8080 (HTTP), generates DKIM keys on first run, logs the required DNS records as structured log lines. In production (`--profile prod`), Caddy starts alongside and handles ports 80/443 with automatic Let's Encrypt certificates.
...
- **TLS termination via Caddy reverse proxy.** Rather than embedding an ACME client in the Go binary, TLS is handled by a Caddy sidecar in the compose stack (`--profile prod`). Caddy provisions and auto-renews Let's Encrypt certificates via HTTP-01, handles the HTTPS→HTTP proxy to rookery on port 8080, and manages HSTS. `run.sh` auto-generates the `Caddyfile` from the domain (and optional contact email) in `rookery.toml` on first `--profile prod` run. ADR-0024 records the full rationale for choosing Caddy over `certmagic`-in-Go.

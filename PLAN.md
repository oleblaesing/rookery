...
- **Format:** a single `.tar.gz.enc` archive (gzip-compressed tar, AES-256-CBC encrypted) containing: (a) `pg_dump --format=custom` of the `rookery` database (`db.dump`), (b) the Caddy TLS certificate volume (`caddy-data.tar`), (c) instance config files (`rookery.toml`, `.env`, `Caddyfile`), and (d) the content-addressed message blob tree (`data/messages/`). **The backup does not — and cannot — contain user PGP private keys**: those live in users' browsers and recovery files (§11.1), not on the server. Restoring from a backup gives users back their mailboxes, addresses, and metadata; users still need their recovery files to actually decrypt past mail. We document this clearly so operators do not develop a false sense of having a complete user-data backup. A "complete" backup from any user's perspective is *the server backup plus that user's own recovery file* — and the recovery file is their responsibility, by design.
...
- **Implementation:** `rookery backup [path]` and `rookery restore <archive>` are subcommands in the dispatcher shell script (pure shell, ~160 lines, no new Go code). The final encrypted output is streaming (tar | gzip | openssl enc); only two intermediate files (`db.dump` and `caddy-data.tar`) are buffered in a temp directory before the pipeline starts. The pipeline ordering rule — `pg_dump` runs before `tar` of `data/messages/` — ensures a mid-backup message arrival produces an orphan blob (harmless) rather than a dangling DB row (broken UI entry).

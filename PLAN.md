...
**Migration to another instance.** Alice on `alice@alice.com` decides to move from instance A to instance B. On instance A she clicks "export archive" in settings; when the background job completes she receives an inbox notification with a download link and a migration deep-link. She uses the migration deep-link: opens it on instance B (replacing "instance-a.example" with "instance-b.example" in the URL), the settings page activates the import tab with the URL pre-filled, she clicks "import" — her browser fetches the encrypted archive from instance A (via instance B's proxy endpoint), decrypts it locally with the private key already in her session, and POSTs the plaintext tar to instance B for ingestion. Her private key was never on instance A — it lives only in the recovery file she controls. Her address, her key, her thread history — all preserved. Correspondents notice nothing. See ADR-0039 for the full design. *(Implemented standalone ahead of Phase 7; address-forwarding and some Phase 7 UX polish remain deferred.)*
...
- **Account deletion flow** per §11.9 (user-initiated web flow with PGP challenge/response confirmation and immediate deletion; operator-initiated variant via `rookery user delete <address>`).
...
- Per-user export of full mailbox (server-side data only — the user's private key has never been on the server, see §11.1; the recovery file the user already holds is the key half of the migration), in the format the migration story uses (Phase 7 will consume this). **Implemented standalone ahead of Phase 6 — see ADR-0039.**
...
- **Full mailbox export from the server**: complete mailbox + addresses + aliases + known-keys cache + metadata + the user's public key. **The export does not contain the private key** — it has never been on the server (§11.1); the user already holds the only copy in their recovery file. Migration is therefore *two* artifacts that the user owns: the mailbox archive (from instance A's export) and the recovery file (which they already have). **Implemented standalone ahead of Phase 7 — see ADR-0039.**
- **Import** on a fresh instance from the mailbox archive. The browser fetches the encrypted archive via instance B's proxy endpoint, decrypts it with the session private key (already in localStorage), and POSTs the plaintext tar to instance B for ingestion (ADR-0039). **Implemented standalone ahead of Phase 7.**
...
- **User-initiated account deletion.** A user can delete their own account from the settings page ("delete account" tab). The flow requires proof-of-key-control: the client signs a server-issued nonce with the user's private key (requiring the recovery file and passphrase), plus a typed-out address confirmation. **There is no grace period** — confirmation deletes immediately in a single transaction. The unread-messages count from the past 24 hours is shown next to the confirm button as an informational tripwire, but it does not block deletion. No tombstone or audit log line is written server-side.
- **What deletion removes.** All messages (encrypted blobs and metadata), the user's stored **public** key, all addresses owned by the user, the user's known-keys cache, all DKIM keys for domains exclusively owned by this user (if no other user uses the domain), all session rows, all drafts, and the user record itself. Blob files exclusively referenced by this user are removed from disk post-commit; blobs shared with other recipients are left intact. There is no encrypted private-key blob to remove — the server never had one (§11.1). Backups created before deletion will still contain the user's server-side data until they age out; we document this. The user's *own* recovery file is theirs to deal with; deleting the account does not reach into their devices.
...
- **Operator-initiated deletion.** The operator can delete a user (e.g. for abuse) via `rookery user delete <address>`, which prompts for typed confirmation in the terminal and then delegates to `rookery-server admin delete-user <address>`. The Go subcommand resolves the address to a user ID and calls the same `lifecycle.DeleteUser` function as the user-initiated web flow — no separate code path, no orphaned blobs.
- **Domain deletion.** Removing a custom domain from the instance requires no other users to be using it for any address. If other users have addresses there, those addresses survive user deletion (the domain row is only removed when it is exclusively owned by the deleted user). We do not orphan-delete addresses behind a user's back.
- **Data retention defaults.** No automatic purge of any mail beyond Trash (§11.5). Sent mail, received mail, drafts: all retained until the user deletes them or the account is deleted. This is policy, not technical limit — operators with regulatory needs can change it. We document it explicitly.
- **GDPR / data-subject requests.** Out of scope to provide a tooling answer for every regulation, but the architecture supports the basics: export (Phase 6 mailbox-export covers right-to-data-portability), deletion (above covers right-to-erasure). Operators in regulated jurisdictions are responsible for their own compliance; we document what the software does and doesn't do.
...
**Backup format and encryption** [shipped in v1, deviates from earlier age shape]

The "single command produces an encrypted, restorable backup" NFR (§10) is implemented as follows:

- **Format:** a single `.tar.gz.enc` archive (gzip-compressed tar, AES-256-CBC encrypted) containing: (a) `pg_dump --format=custom` of the `rookery` database (`db.dump`), (b) the Caddy TLS certificate volume (`caddy-data.tar`), (c) instance config files (`rookery.toml`, `.env`, `Caddyfile`), and (d) the content-addressed message blob tree (`messages-data.tar`, archived from the `messages-data` volume). **The backup does not — and cannot — contain user PGP private keys**: those live in users' browsers and recovery files (§11.1), not on the server. Restoring from a backup gives users back their mailboxes, addresses, and metadata; users still need their recovery files to actually decrypt past mail. We document this clearly so operators do not develop a false sense of having a complete user-data backup. A "complete" backup from any user's perspective is *the server backup plus that user's own recovery file* — and the recovery file is their responsibility, by design.
- **Encryption:** the archive is encrypted with **openssl enc -aes-256-cbc -pbkdf2 -salt** using a passphrase the operator provides at backup time (prompted on the terminal, or read from a `--passphrase-file` for automation). The passphrase is the single secret required to decrypt the backup; it is never written to disk by the dispatcher and is passed to openssl only via an inline environment-variable assignment (not a flag argument, to avoid shell-history leaks).
- **Why passphrase over age:** the earlier-committed age shape (see below) required an age recipient keypair: the operator would need to generate, store, and configure an age public key in `rookery.toml`, and keep the corresponding private key somewhere safe. A symmetric passphrase removes this setup step and the corresponding `[backup]` config block. The operator's mental model becomes a single secret ("remember this passphrase, protect this file") rather than two artifacts (config block + separate key file). The threat model is unchanged: a server-compromise attacker who can read the filesystem can read the passphrase-file (if cron uses `--passphrase-file`) or intercept the passphrase in memory; the backup still protects offsite copies against a storage-provider attacker. *Deviation from earlier shape:* the plan previously committed to `age` and a `[backup]` config block (ADR-0029). That shape was revised before Phase 6 code landed. The `age` tool was not added to the project; `openssl` was already a prerequisite. No ADR-0029 document was written; this paragraph is the retroactive rationale.
...
- **Implementation:** `rookery backup [path]` and `rookery restore <archive>` are subcommands in the dispatcher shell script (pure shell, ~160 lines, no new Go code). The final encrypted output is streaming (tar | gzip | openssl enc); the intermediate files (`db.dump`, `caddy-data.tar`, and `messages-data.tar`) are buffered in a temp directory before the pipeline starts. The pipeline ordering rule — `pg_dump` runs before the message blobs are archived — ensures a mid-backup message arrival produces an orphan blob (harmless) rather than a dangling DB row (broken UI entry).
- **Automated verification:** the test harness (`internal/lifecycle/backup_test.go`) seeds a user + message + blob, creates an encrypted archive using the same tar/gzip/openssl pipeline, decrypts it, and asserts all files are present and blob bytes are identical. A second test performs a full `pg_dump` → `pg_restore` round-trip into a temp database, asserting user and message rows are present. Both tests run via `./rookery test` (the golang test container includes `postgresql-client` for pg_dump/pg_restore). No dependence on a hosted CI provider, in keeping with §7.
- **Not in scope:** incremental backups, retention policy, rspamd/redis training data (caches that relearn from traffic), any web UI for backup management.
...
| `rookery backup [path]` | Create an encrypted backup (pulled forward from Phase 6). | Shell pipeline: `pg_dump \| tar \| gzip \| openssl enc -aes-256-cbc -pbkdf2`. Intermediate files for DB dump and caddy-data are staged; final output streams. See §11.10 for the full shape and the deviation from the earlier-committed age encryption. |
| `rookery restore <archive>` | Restore from a backup archive on a fresh checkout (pulled forward from Phase 6). | Inverse pipeline: `openssl dec \| gunzip \| tar -x`, then `pg_restore`, then caddy-data volume restore. Refuses to run on a non-fresh instance. |
...
| `rookery user delete <address> [reason]` | Operator-initiated deletion (§11.9) | prompts for typed address confirmation in the terminal, then delegates to `rookery-server admin delete-user <address>` which calls `lifecycle.DeleteUser` |
...
Subcommand names in the Phase 5 row are illustrative — flat verbs (`suspend-user`, `print-dns`) are accepted where they read better. The noun-verb preference is documented in ADR-0033, not a hard rule. Phase 8 may add `rookery relay-client create` if the relay-rookery shape (ADR-0030) is built.
...
**User portability:**
- `ADR-0039` — Per-user data export/import (instance portability). Async export with inbox delivery, PGP-encrypted to the user's own key, import decrypts in the browser. Implemented standalone ahead of Phase 6/7. **Written; Phase 4 follow-up.**
